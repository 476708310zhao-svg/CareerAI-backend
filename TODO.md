...
> 最后整理：2026-09-09。状态以勾选框和每个任务正文为准。优先保证可安全发布和核心链路真实跑通。
...
- [x] Sprint 6 OA、证据型项目与岗位可信度
- [x] Sprint 7 商业化治理与 Mock 灰度准备
...
- [x] TASK-007 优化提醒派发慢请求并验证幂等
- [ ] TASK-008 配置生产外部错误告警和发布监控
- [x] TASK-009 验证跨设备 Today/收藏/校招提醒一致性（自动化完成，真实双设备验收并入 TASK-005）
- [x] TASK-010 落地统一数据库 migration 基线机制（生产 baseline 登记待 Human 上线审批）
- [ ] TASK-011 分批迁移非必要的直接小程序网络请求（收藏链路已完成）
...
目标：完成提醒派发可靠性、Today/收藏跨设备一致性和收藏请求层统一，不执行真实推送或生产变更。

任务：

- [x] 提醒派发增加唯一投递键、90 秒租约、最多 5 次重试和不重复站内消息
- [x] 增加 batch、游标续跑、并发上限 5 及 checked/due/sent/failed/skipped/durationMs 埋点
- [x] Today 改为增量合并，不再删除另一设备未上报的任务；旧时间戳不能覆盖新状态
- [x] 小程序 Today 仅在对应请求版本仍有效时清除 pending，避免飞行中请求误确认新操作
- [x] 收藏增加本地 outbox、服务端操作回执和删除 tombstone，离线重试沿用 operationId
- [x] 收藏工具和收藏页普通请求迁入 `api-client`，不再直接调用 `wx.request`
- [x] Sprint 8 专项 6/6、Smoke 72/72、全量 202/202 和小程序检查通过

下一批：继续 TASK-011，优先迁移首页和普通 AI JSON 请求，并明确上传、SSE、LeetCode 等例外；真实双设备与订阅消息 staging 验收并入 TASK-005，生产 migration 仍需 Human 审批和备份演练。
...
优先级：P1  
状态：自动化安全基线完成；真实供应商 staging 抽样待 Human 配置 Key 与预算
所属模块：AI
...
- [x] 所有错误类型均在故障注入中按约定降级或失败
- [x] 自动门禁验证不泄露直接 PII、不绕过确认、拦截虚构执行与无证据数字
- [ ] 真实供应商质量/延迟/成本达到书面阈值
- [x] kill switch 无外部请求回滚验证通过
...
任务：优化提醒派发慢请求并验证幂等  
优先级：P1  
状态：代码与自动化验证完成（2026-09-09）；真实微信 staging 失败恢复验收待 Human
所属模块：通知/性能
...
- [x] 本地/测试派发改为日期窗口扫描、batch/游标续跑、有界并发并记录结构化耗时
- [x] 同一提醒由唯一 delivery key 与租约保证不重复发送
- [x] 部分失败可重试，站内消息不会重复，错误只保存安全类别
...
优先级：P1  
状态：代码与双会话自动化验证完成（2026-09-09）；真实双设备验收并入 TASK-005
所属模块：Today/提醒
...
- `miniprogram/utils/daily-tasks.js`
- `miniprogram/utils/favorites.js`
- `routes/favorites.js`

完成标准：

- [x] Today 和收藏使用服务端最终状态与更新时间冲突控制，双会话自动化通过
- [x] 离线收藏沿用唯一 operationId，Today 只确认对应本地版本
- [x] 截止时间变化生成新 delivery key，不受旧日期记录阻塞
...
优先级：P1  
状态：已完成自动化基线与隔离演练（2026-09-04）；生产登记待 Human
所属模块：数据库
...
- `miniprogram/utils/api-favorites.js`

完成标准：

- [ ] 普通业务页面不再自行处理重复的 token/401/网络错误
- [ ] 保留例外有代码注释和测试
- [x] 收藏工具与收藏页已迁移，专项和小程序发布检查通过
...
- [x] 2026-09-08 Sprint 6 OA、证据型项目与 Job Trust Score：本人确认训练统计、四里程碑留证、真实成果经历库写入、五项岗位风险证据；190/190 tests
