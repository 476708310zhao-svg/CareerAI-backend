...
目标：完成 OA 训练闭环、证据型项目补强和岗位可信度辅助判断。

任务：

- [x] OA 按公司/岗位/题型建计划，支持单一进行中计时练习、错题本和题型统计
- [x] OA 成绩、耗时和错题必须由本人确认；无样本时显示“不评估”
- [x] 项目按岗位差距生成四个里程碑、数据来源、交付物和验收标准，来源为可审计规则
- [x] 每个里程碑完成必须留证；项目完成必须确认真实成果和可核验交付物
- [x] 未完成项目不能写简历，简历数字必须存在于完成证据
- [x] Job Trust Score 展示五项证据、风险、置信度和本人官网核验状态，不冒充系统联网验证
- [x] Sprint 6 专项 7/7、migration 5/5、全量 190/190 和 `npm run check:release` 通过

下一批：Sprint 7 商业化灰度的方案与审批准备。真实支付、生产 migration、真实供应商 staging 和部署仍需 Human 明确审批。
