...
> 最后整理：2026-09-08。状态以勾选框和每个任务正文为准。优先保证可安全发布和核心链路真实跑通。
...
目标：完成商业化代码治理与 Mock 灰度准备，不开启真实支付或生产放量。

任务：

- [x] 保持历史 planId 0～3 语义，新增 JD 简历包、7 天面试冲刺包和秋招季度包
- [x] Free/Pro/场景包权益分层，场景包只叠加指定额度，不提升完整 VIP
- [x] 订单创建、支付确认、权益发放、额度使用、退款审核与撤销进入幂等追加账本
- [x] 退款必须本人确认、管理员审核；完成必须记录已执行的外部凭据并撤销对应权益
- [x] 建立 readiness、5xx、AI 降级、cron、支付回调和权益异常六类告警
- [x] 商业灰度默认 0%，无 `COMMERCE_ROLLOUT_APPROVED=true` 不能提高比例
- [x] 小程序会员页展示 Pro/场景包边界、当前权益、订单和退款入口
- [x] Sprint 7 专项 4/4、migration 5/5、Smoke 71/71、全量 195/195 和 `npm run check:release` 通过

下一批：优先使用匿名真实样本校准核心闭环和提醒节奏；如要继续真实商业化，必须由 Human 先完成主体资质、商品、退款方案、生产备份与 migration 审批，再单独进行支付沙箱和小流量灰度。
