...
router.put('/', authMiddleware, (req, res) => {
  try {
    ...
    if (!profile) return fail(res, '用户不存在', 404);
    analytics.track(req.user.userId, profile.completion >= 80 ? 'profile_completed' : 'profile_started', withCoreRefs(
...
router.get('/completion', authMiddleware, (req, res) => {
  ...
  return ok(res, { completion: profile.completion, missing, refs: profile.refs });
});
