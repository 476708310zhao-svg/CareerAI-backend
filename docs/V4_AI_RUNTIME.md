...
示例配置为 `V4_AI_LIVE_ENABLED=false`。显式关闭时不会发送外部模型请求，接口使用已测试的确定性规则结果，适合本地开发、自动化测试和供应商故障降级。若未设置该变量但已配置有效供应商密钥，运行时会启用真实模型；仍可随时通过显式设置 `false` 作为紧急关闭开关。
...
- 模型无权直接执行数据库写操作；
- 简历建议必须准确引用原简历内容；
- 简历建议必须与原文有实质差异，姓名、联系方式、年龄、性别和系统时间戳不得伪装成润色建议；
