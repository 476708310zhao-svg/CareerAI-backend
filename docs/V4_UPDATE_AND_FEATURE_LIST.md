...
- 🟡 已通过 69/69 测试、小程序检查和微信 preview
- 🟡 相关首页文件尚未纳入 `d541cb6`，需要单独审查和提交
...
- ✅ Today 任务完成事件已埋点
- 🟡 首页本地任务和 V4 服务端 `today_tasks_v4` 尚未完全统一
- ⬜ 需要建立统一任务优先级、过期规则和跨设备同步
...
- ⬜ 为 Agent、申请材料、简历建议和面试评分接入真实 AI 模型
- ⬜ 完成真实模型超时、失败、重试和降级测试
...
1. 先审查并提交 Sprint 3 首页工作台。
2. 统一 Today 任务的数据源和跨设备同步。
3. 接入真实 AI 模型，并保留当前规则逻辑作为降级方案。
4. 重新登录微信开发者工具，完成 11/11 真实 E2E。
5. 完成真机验收和生产配置检查。
6. 保持真实支付关闭，先从 5% V4 功能灰度开始。
