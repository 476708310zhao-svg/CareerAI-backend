...
- ✅ 首页已重构为求职工作台
- ✅ 已拆分 Today 计划、核心工具、推荐岗位和每日最新校招组件；求职情报已从首页移至资源中心
- ✅ 已通过 91/91 测试和小程序检查；此前首页微信 preview 已通过
...
- ✅ 一级导航已替换为 `首页 / 资源 / 进度 / AI Career / 我的`
- ✅ 资源中心按“校招机会 / 面试与笔试 / 职业决策 / 内容与服务”重组，聚合面试题库、STAR 案例、薪酬查询、公司情报、OA 题库、AI 模拟面试、技能路径、职业洞察、校招日历、求职资讯、机构测评和使用指南
- ✅ 资源中心按产品参考图完成自定义导航、大标题、校招日历主视觉、横向双列工具卡与内容服务列表的统一视觉重构
- ✅ 校招日历保留为完整二级页面；首页每个岗位独占一行，仅展示最新业务日期并按更新时间倒序，原岗位推荐页保留为二级搜索与推荐入口
...
- ✅ 保留全局 AI 悬浮入口直达通用 AI 助手；首页会员权益横幅在支付未开放时展示明确状态且不进入支付链路
- ⬜ 收藏岗位更新提醒需要完整回归
