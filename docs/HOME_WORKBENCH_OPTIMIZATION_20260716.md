...
- 本阶段不调整一级 TabBar；V4 目标导航仍需在“Today / Jobs / Applications / AI Career / Profile”方案确认后单独实施。
- 全局自定义 TabBar 已提供 AI 悬浮助手，首页不需要重复保留“AI 助手”宫格入口。
...
| 10 宫格 | 与 TabBar、求职计划和 AI 悬浮入口重复 | 从首页移除，不删除业务页面 |
| 今日求职简报 | 与今日求职工作台重复 | 合并到今日求职计划 |
| 今日校招信息更新 | 原长列表影响首屏，但它是核心求职机会入口 | 首页保留精选：1 条重点 + 最多 2 条更新，完整内容进入“校招”Tab |
| 会员权益 | 固定营销内容打断主线 | 移至“我的”及功能触发场景 |
...
- 已移除首页 10 宫格、重复简报、会员卡和热门企业展示，并将校招长列表收敛为精选核心模块。
...
- 候选 TabBar 调整未纳入本阶段提交，等待 V4 一级导航方案确认。
- 验证：`npm test` 76/76 通过，`npm run check:miniprogram` 通过。
- 微信开发者工具 `preview` 编译通过，预览包总计约 2.0 MB，主包约 676.1 KB。
