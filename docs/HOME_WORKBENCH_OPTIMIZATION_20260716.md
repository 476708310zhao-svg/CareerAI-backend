...
- V4 一级 TabBar 已调整为“首页 / 资源 / 进度 / AI Career / 我的”。
...
- `home-campus-updates`：每日最新校招单列信息流及完整校招入口。
...
| 今日校招 | `getCampusList` 与本地缓存，仅取最新业务日期并按更新时间倒序，首页最多展示 8 条单列卡片 |
...
4. 将首页岗位限制为最多 3 条、校招日历限制为最多 8 条。
5. 保留顶部搜索框和 Banner 轮播，停止热门企业的首页请求；保留校招缓存与延迟加载。
6. 为岗位与校招补齐独立 loading / empty / error / retry。
7. 将一级导航调整为首页、资源、进度、AI Career、我的，并迁移旧 switchTab 跳转。
...
- 首页不再出现 10 宫格、求职情报和热门企业；会员权益以紧凑横幅呈现。
- 今日求职计划是首页唯一求职状态模块。
- 四个核心工具均可进入现有真实页面。
- 今日校招展示真实接口数据，支持进入详情、校招日历和完整列表。
- 推荐岗位不超过 3 条，每日最新校招不超过 8 条。
- 单个接口失败不会导致首页白屏。
...
- 首页请求 Banner、推荐岗位与校招精选；求职情报请求已移除，各模块均限制首屏数据量。
- 首页按参考图重做为白底、浅蓝描边、低阴影、紧凑行高与蓝绿状态色的统一视觉语言。
- 推荐岗位保持紧凑列表，今日校招改为每个岗位独占一行，并按更新时间由新到旧展示。
- 保留岗位与校招的独立 loading / empty / error / retry 状态。
- 登录用户的首页任务已与服务端 Today 任务幂等合并；未登录或弱网时继续使用本地任务。
- 本地完成状态支持离线保存和联网补传，服务端 AI/面试任务不会被首页同步删除。
- V4 一级导航已落地；资源 Tab 聚合题库、STAR、薪酬、公司、OA、校招、资讯和职业探索等入口，校招日历与岗位推荐保留为二级页面。
- 新增 Progress 主包求职进度工作台与 AI Career 四 Agent 主包入口。
- 按产品要求恢复全局 AI 悬浮入口；会员开关关闭时横幅仍保留，但明确提示真实微信支付未开放。
- 验证：`npm run check:release` 通过，91/91 tests、内容错误 0，主包估算约 0.96 MB。
