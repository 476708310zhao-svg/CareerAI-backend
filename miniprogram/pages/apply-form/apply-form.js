...
Page({
  data: {
    source: '', slug: '', jobId: '', title: '', company: '',
    step: 1,
  ...
  onLoad(query) {
    ...
    this.setData({ source, slug, jobId, title, company });
  ...
  _loadPdfs() {
    ...
    getMyPdfs().then(res => {
      const pdfs = (res && res.data) || [];
      this.setData({
        pdfs,
        pdfsLoading: false,
        selectedPdfId: pdfs.length > 0 ? String(pdfs[0].id) : '',
      });
    }).catch(() => this.setData({ pdfsLoading: false }));
  },
