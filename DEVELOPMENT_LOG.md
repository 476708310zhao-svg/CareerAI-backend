...
## 2026-09-08｜Sprint 7：商业化治理与 Mock 灰度准备

### 本次完成

- 保持历史 planId 0～3 的月卡、季卡、年卡和体验会员语义，在 4～6 新增 JD 简历包、7 天面试冲刺包和秋招季度包。
- Free/Pro/场景包权益明确分层；场景包只叠加指定额度，不会把用户提升为完整 VIP。
- 建立订单创建、支付确认、权益发放、额度使用、退款申请/审核/完成和权益撤销的幂等追加账本。
- 退款状态机要求本人确认、管理员审核；完成退款必须填写已执行的外部凭据，随后撤销 Grant、取消对应订阅并重算会员状态。
- 新增 readiness、15 分钟 5xx、AI 降级率、cron 最近失败、支付回调拒绝和已支付缺权益六类告警，支持确认、解决与自动恢复。
- 商业灰度默认 0%；未设置 `COMMERCE_ROLLOUT_APPROVED=true` 时，管理员也不能提高比例。
- 小程序会员页增加 Pro/场景包说明、当前权益包、订单列表和退款申请入口，并修复场景包错误写入本地 VIP 的风险。

### 技术决策

- Sprint 7 migration 仅新增商业账本、权益 Grant 与告警三张表及索引，不改写或删除历史业务表。
- 下单与订单创建账本在同一事务；支付确认、Grant 和订阅写入幂等，旧已支付订单缺 Grant 时按历史到期信息补账。
- 代码只记录外部退款已经完成的凭据，不调用任何真实退款 API；运营概览固定声明 `paymentLive=false`。
- 已确认的告警在信号持续时保持 acknowledged，信号消失后自动 resolved，避免重复评估抹掉人工处理状态。

### 验证

- Sprint 7 专项 4/4，migration 专项 5/5，服务端 Smoke 71/71，全量 195/195。
- `npm run check:release` 通过：AI 故障矩阵 7/7、外部请求 0、小程序媒体 191.2 KB、数据检查 Errors 0 / Warnings 4。
- 小程序静态检查、JavaScript 语法检查和 `git diff --check` 通过。
- 未运行真实微信 E2E，未调用真实 AI，未操作生产数据库、真实支付或外部退款，未推送、未部署。

### 遗留门禁

- 主体资质、虚拟商品、退款政策、隐私与客服方案必须先获 Human 审批。
- 生产 migration 必须先完成明确目标、备份、恢复验证和 checksum 审核。
- 真实支付只能在独立批次进入沙箱，再按 0%→5% 小流量并观察 30～60 分钟；任何异常可立即回到 0%。

### 下一步

- 默认进入匿名真实样本校准与核心体验收口；如 Human 决定商业上线，再单独执行资质审批、生产 migration、支付沙箱和小流量灰度。

---

## 2026-09-09｜Sprint 8：提醒可靠性与跨设备一致性

### 本次完成

- 提醒派发新增唯一投递键、租约、失败重试、日期窗口扫描、游标续跑和有界并发。
- Today 改为跨设备增量合并并拒绝旧客户端状态覆盖。
- 收藏新增 outbox、操作回执和删除 tombstone，并迁入统一请求客户端。

### 验证

- 专项 6/6、Smoke 72/72、全量 202/202；小程序检查与差异检查通过。
- 真实微信请求 0；未运行真实 E2E，未调用真实 AI，未改生产数据库，未推送或部署。

### 下一步

- 继续迁移首页普通 JSON 请求；上传、SSE 和第三方直连保留明确例外。
- 真实双设备、微信 staging 和生产 migration 由 Human 审批后验收。

## 2026-09-09｜竞争力陪跑空白页面修复

### 本次完成

- 新增竞争力仪表盘响应解析与完整性校验；只有包含七维诊断、周报、3/6/12 月计划和 Today 数据的成功响应才进入内容视图。
- 登录失效时清理旧仪表盘状态并展示重新登录入口；超时、断网、限流、HTTP 错误和残缺数据展示明确的可重试错误。
- 修复通用 GET 请求降级结果被页面当成成功对象后，出现 `/100`、空七维、空周报和空计划的问题。

### 验证

- Sprint 4 专项 6/6、全量 203/203、AI 故障矩阵 7/7、小程序和数据检查通过。
- 未运行真实 E2E、未调用真实 AI、未部署；小程序连接的生产后端仍需单独完成版本发布。
